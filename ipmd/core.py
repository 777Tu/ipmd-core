...
class RIPIA:
    def __init__(self, imgFile, infos=None):
        if not Path(imgFile).exists():
            raise ValueError("Image not found.")
        if infos is None:
            infos = {
                "_Time_": time.strftime("|%m/%d/%Y|"),
                "_Name_": "Unkown|"}
        if not isinstance(infos, dict):
            raise TypeError(
                "\nInvalid data type. 'info' must be a dictionary.")
        if ("_Time_" not in infos or "_Name_" not in infos) or (not infos["_Time_"].startswith(
                "|") or not infos["_Time_"].endswith("|") or not infos["_Name_"].endswith("|")):
            raise ValueError(
                "Error: Incorrect information format. \nExpected: \n\t\"{'_Time_': '|M/D/Y|', '_Name_': 'object-name|'}\". \nMake sure you put the pipe '|' in the right spots so your data can be retrieved correctly.")
        if len(
            infos["_Time_"]) > 15 or len(
            infos["_Time_"]) < 8 or len(
            infos["_Name_"]) < 5 or len(
                infos["_Name_"]) > 30:
            raise TypeError(
                "Unexpected Date or Name. Lenght Date must be between 15&8. Lenght Name must be between 30&5.")
    ...
    def AsciiJoinedChunk(self):
        ...
        asciiJoined = [
            char for bundle in asciiChunk for chunk in bundle for char in chunk]
    ...
    def AsciiToPixel(self, listChunk3Pixel):
        if not isinstance(listChunk3Pixel, list):
            return "pixel container must be list[]"
        infoLength = self.TotalInfoPixel()
        pixelLength, givenLength = math.ceil(
                infoLength / 3), len(listChunk3Pixel[0])
        if len(listChunk3Pixel) != 3:
            return "pixel chunk must be equal to 3"
        if givenLength < pixelLength:
            return f"{givenLength} pixel length given, expected length {pixelLength}"
        asciilistPixel = []
...
class RIPIAR(RIPIA):
    ...
    def reveal(self):
        ...
        _widthCenter = math.ceil(
            (_imgWidth / 2) - (_staticLenght / 2)) if _imgWidth // 2 >= _staticLenght else 0
        ...
        while _3Map_ > 0:
            ...
            _3Map_ -= 1
        _directPCharPip_ = [
            chr(char) for bundle in _directPInfo_ for pixel in bundle for char in pixel]
        _3MapInfo_ = []
        for bundle in _directPInfo_:
            ...
            currentBundle1DStr = currentBundle1DStr[currentBundle1DStr.find(
                "124"):currentBundle1DStr.rfind("124") + 3]
            currentBundle1DIntList = [
                int(char) for char in currentBundle1DStr.split(',')]
            _each3MapInfo_ = {}
            ...
            _3MapInfo_.append(
                ["".join(map(str, [tLchr.lower() for tLchr in _each3MapInfo_["_Info_"]]))])
            _each3MapInfo_ = {}
...
def ArgParser():
    ...
    parse.add_argument(
        "-ach",
        "--anchor",
        help="Use this to embed info into the image pixels.",
        action="store_true")
    parse.add_argument(
        "-src",
        "--source",
    ...
    parse.add_argument(
        "-info",
        "--information",
        help="The dictionary of info you want to hide (use '...' for the dict).",
        type=literal_eval)
    parse.add_argument(
        "-sv",
        "--save",
        help="Custom name to save the output file (optional).",
        default=True)
    parse.add_argument(
        "-r",
        "--retrieve",
        help="Use this to extract hidden info from an image.",
        action="store_true")
    parse.add_argument(
        "-rsrc",
        "--retrievesource",
        help="The image file you want to extract info from.")
